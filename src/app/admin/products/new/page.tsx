...
const emptyImages: ImageJson = { thumbnails: [""], previews: [""
] };
...
const NewProductPage: React.FC = () => {
  ...
  const [imgs, setImgs] = useState<ImageJson>(emptyImages);
  ...
  useEffect(() => {
    ...
    fetch("/api/categories")
      .then((r) => r.json())
      .then((data) => {
        const cats = (data || []).map((c: any) => ({ id: c.id, title: c.title, slug: c.slug }));
        setCategories(cats);
      })
      .catch(() => {});
  }, []);
  ...
  const canSave = useMemo(() => {
    return title.trim().length > 0 && description.trim().length > 0 && price > 0;
  ...
  const addImageField = (key: keyof ImageJson) => {
    setImgs((prev) => ({ ...prev, [key]: [...prev[key], ""] }));
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-1 p-6">
        <h1 className="text-2xl font-semibold text-dark mb-2">New Product</h1>
        <p className="text-gray-6">Enter details and image URLs. You can add multiple items in each section.</p>
      </div>
