...
const SingleItem = ({ item, onNotifyMe }: { item: Product; onNotifyMe?: (product: Product) => void }) => {
  ...
  // This function ensures the correct product data is stored before navigating.
  ...
  return (
    <div className="group">
      <div className="relative overflow-hidden rounded-lg bg-[#F6F7FB] min-h-[403px]">
        ...
        <div className={`absolute right-0 bottom-0 flex flex-col gap-2 p-5.5 ease-linear duration-300 ${
            item.inStock 
            ? 'translate-x-full group-hover:translate-x-0' 
            : 'translate-x-0' // Keep visible if out of stock
        }`}>
          {item.inStock ? (
            <>
              <button
