...
async function createBackup(): Promise<string> {
  ...
  const backupFileName = `pre-restore-backup-${timestamp}.sql`;
...
async function checkDataExists(): Promise<boolean> {
  try {
    ...
    return productCount > 0 || categoryCount > 0 || blogCount > 0;
  } catch (error) {
  ...
  try {
    ...
    if (isProductionEnvironment()) {
      ...
      if (!canRunInProduction()) {
        ...
        console.log('');
        console.log('Example:');
        console.log('  ALLOW_PRODUCTION_RESTORE=true PRODUCTION_RESTORE_CONFIRMATION=I_UNDERSTAND_THIS_WILL_DELETE_ALL_DATA npx tsx scripts/restore-all-data.ts');
        console.log('');
        console.log('❌ Restoration aborted for safety.');
        return;
      }
      
      console.log('✅ Production restoration confirmed via environment variables');
    }
    
    // Safety check 2: Check for existing data
    const hasExistingData = await checkDataExists();
    if (hasExistingData) {
      console.log('⚠️  WARNING: Existing data found in database!');
      
      if (isProductionEnvironment()) {
        console.log('🔄 Production mode: Creating backup before deletion...');
      } else {
        console.log('🔄 Development mode: Creating backup before deletion...');
      }
    }
    ...
    try {
      ...
      console.log('❌ Backup creation failed. Aborting restoration for safety.');
      console.log('   Error:', error.message);
      return;
    }
    
    console.log('✅ All safety checks passed. Proceeding with data restoration...');
      ...
      data: [
        {
          ...
          createdAt: new Date('2025-09-02 04:50:49.677'),
          updatedAt: new Date('2025-09-14 08:24:25.518')
        },
        {
          id: 2,
          title: 'Tinctures',
    ...
    console.log(`✅ Restored ${categories.count} categories`);
    ...
    console.log(`✅ Restored ${products.count} products`);

    // Restore Product-Category Relationships
    console.log('🔗 Restoring product-category relationships...');
    const productCategories = await prisma.productCategory.createMany({
      data: [
        { id: 1, productId: 1, categoryId: 1 },
        { id: 2, productId: 1, categoryId: 3 },
        { id: 3, productId: 2, categoryId: 2 }
      ]
    });
    console.log(`✅ Restored ${productCategories.count} product-category relationships`);
      ...
      data: [
        {
          ...
          views: 100000,
          published: true,
          createdAt: new Date('2025-09-02 04:51:23.347'),
          updatedAt: new Date('2025-09-02 04:51:23.347'),
    ...
    console.log(`✅ Restored ${blogPosts.count} blog posts`);

    // Restore Training Programs
    console.log('🎓 Restoring training programs...');
    const trainingPrograms = await prisma.trainingProgram.createMany({
      data: [
        {
          id: 1,
          name: 'Oyster Mushroom Training',
          slug: 'oyster-mushroom-training',
          description: 'dsfsadfasfldsakjf',
          price: 5000,
          duration: 10,
          dailyHours: '5-6 Hours',
          type: 'OYSTER',
          isActive: true,
          createdAt: new Date('2025-09-13 17:17:07.529'),
          updatedAt: new Date('2025-09-15 14:11:25.083')
        }
      ]
    });
    console.log(`✅ Restored ${trainingPrograms.count} training programs`);

    console.log('\n🎉 Comprehensive data restoration completed successfully!');
    console.log('📊 Summary:');
    console.log(`   - Categories: ${categories.count}`);
    console.log(`   - Products: ${products.count}`);
    console.log(`   - Product-Category Relationships: ${productCategories.count}`);
    console.log(`   - Blog Posts: ${blogPosts.count}`);
    console.log(`   - Training Programs: ${trainingPrograms.count}`);
